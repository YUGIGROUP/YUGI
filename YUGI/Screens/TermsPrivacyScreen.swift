...
struct TermsPrivacyScreen: View {
    ...
    @State private var showSegment = false
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 0) {
                    legalSegmentButton("Terms", tag: 0)
                    legalSegmentButton("Privacy", tag: 1)
                }
                .padding(4)
                ...
                .padding(.top, 20)
                .padding(.bottom, 16)
                .opacity(showSegment ? 1 : 0)
                .offset(y: showSegment ? 0 : 12)
                .animation(.easeOut(duration: 0.6), value: showSegment)
        ...
        .onAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.22) { showSegment = true }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.38) { showContent = true }
        }
    }

    @ViewBuilder
    private func legalSegmentButton(_ title: String, tag: Int) -> some View {
        Button {
            ...
            Text(title)
                .font(selectedTab == tag ? .custom("Raleway-Medium", size: 13) : .custom("Raleway-Regular", size: 13))
                .foregroundColor(selectedTab == tag ? Color.yugiSoftBlack : Color.yugiBodyText)
...
// MARK: - Prose Section (used in redesigned Terms/Privacy views)

struct TermsProseSection: View {
    let title: String
    let content: String
...
struct TermsOfServiceView: View {
    ...
    private var acceptanceSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                Button(action: { hasAcceptedTerms.toggle() }) {
                    Image(systemName: hasAcceptedTerms ? "checkmark.square.fill" : "square")
                        .font(.system(size: 24))
                        .foregroundColor(hasAcceptedTerms ? Color.yugiMocha : Color.yugiBodyText.opacity(0.6))
                }
                .buttonStyle(.plain)
            ...
            .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.yugiOat, lineWidth: 1))
            .cornerRadius(12)
            ...
            Button(action: {
                ...
                .frame(maxWidth: .infinity)
                .padding(.vertical, 14)
                .background(hasAcceptedTerms ? Color.yugiMocha : Color.yugiBodyText.opacity(0.3))
                .clipShape(Capsule())
            }
...
// MARK: - Legacy card-style section (used by TermsAgreementScreen — do not remove)
...
// MARK: - Legacy tab button (used by TermsAgreementScreen — do not remove)
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 0) {
                    TermsPrivacyTabButton(
                        ...
                        withAnimation(.easeInOut(duration: 0.3)) { selectedTab = 0 }
                    }
                    ...
                    TermsPrivacyTabButton(
                        ...
                        withAnimation(.easeInOut(duration: 0.3)) { selectedTab = 1 }
                    }
                }
                .background(
