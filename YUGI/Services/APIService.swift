...
class APIService: ObservableObject, @unchecked Sendable {
    ...
    func fetchRecommendedClasses(latitude: Double, longitude: Double, category: String? = nil, preferredDays: String? = nil, radiusKm: Int? = nil) -> AnyPublisher<ClassesResponse, APIError> {
        var queryItems = [
            "recommend=true",
            "latitude=\(latitude)",
            "longitude=\(longitude)"
        ...
        // Search radius in km; omitted when nil so the backend applies its default.
        if let radiusKm = radiusKm {
            queryItems.append("radiusKm=\(radiusKm)")
        }
